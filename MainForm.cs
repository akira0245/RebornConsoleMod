...
namespace RebornConsoleMod
{
    public partial class MainForm : Form
    {
        ...
        private void MainForm_Load(object sender, EventArgs e)
        {
            ...
            SetupComboBox();
            csharpCode.LanguageOption = RichTextBoxLanguageOptions.UIFonts;
            luaCode.LanguageOption = RichTextBoxLanguageOptions.UIFonts;
            csharpOutput.LanguageOption = RichTextBoxLanguageOptions.UIFonts;
            luaOutput.LanguageOption = RichTextBoxLanguageOptions.UIFonts;
        }
        ...
        private void btnKeybind_KeyUp(object sender, KeyEventArgs e)
        {
            if (_keyBindMode)
            {
                ...
                if (_cachedKey != Keys.None && _cachedKey != Keys.Escape)
                {
                    ...
                    HotkeyManager.Unregister("RebornConsoleMod");
                    HotkeyManager.Register("RebornConsoleMod", RebornConsoleSettings.Instance.Key, RebornConsoleSettings.Instance.ModifierKey, hotkey => { invokeCompile(); });
                }
                SetKeybindButtonToNormalMode((Button)sender);
            }
        }
        ...
        private void 自动换行ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            csharpOutput.WordWrap = (sender as ToolStripMenuItem).Checked;
            luaOutput.WordWrap = (sender as ToolStripMenuItem).Checked;
        }

        private void 自动换行ToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            (sender as ToolStripMenuItem).Checked = !(sender as ToolStripMenuItem).Checked;
            csharpCode.WordWrap = (sender as ToolStripMenuItem).Checked;
            luaCode.WordWrap = (sender as ToolStripMenuItem).Checked;
        }

        private void 布局切换ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (RebornConsoleSettings.Instance.TabIndex == 0)
            {
                Instance.splitContainer1.Orientation = Instance.splitContainer1.Orientation == Orientation.Horizontal
                    ? Orientation.Vertical
                    : Orientation.Horizontal;
            }
            else
            {
                Instance.splitContainer2.Orientation = Instance.splitContainer2.Orientation == Orientation.Horizontal
                    ? Orientation.Vertical
                    : Orientation.Horizontal;
            }
        }

        private void 布局切换ToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            if (RebornConsoleSettings.Instance.TabIndex == 0)
            {
                Instance.splitContainer1.Orientation = Instance.splitContainer1.Orientation == Orientation.Horizontal
